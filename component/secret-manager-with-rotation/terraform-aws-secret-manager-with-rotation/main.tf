locals {
  ...
  lambda_description = var.rotation_type == "single" ? "Conducts an AWS SecretsManager secret rotation for RDS Aurora PostgreSQL using single user rotation scheme" : "Conducts an AWS SecretsManager secret rotation for RDS Aurora PostgreSQL using multi user rotation scheme"
  ...
  secret_string_single_replica = {
    ...
    port        = var.postgresql_port
    dbname      = var.postgresql_dbname
    replicahost = var.postgresql_replicahost
  }
  secret_string_single = var.postgresql_replicahost == null ? local.secret_string_single_bare : local.secret_string_single_replica
  ...
  secret_string_multi = var.postgresql_replicahost == null ? local.secret_string_multi_bare : local.secret_string_multi_replica
}
...
resource "aws_secretsmanager_secret" "default" {
  ...
  description = "Username and password for RDS user [${var.postgresql_username}]."
